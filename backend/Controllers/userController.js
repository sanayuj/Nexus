...
const notificationModel = require("../Models/adminNotification");
...
module.exports.userFeedback = async (req, res, next) => {
  try {
    const feedbackExists = await userFeedbackModel.findOne({
      userId: req.params.userId,
    });
    if (feedbackExists) {
      return res.json({
        message: "Your feedback already exists",
        status: false,
      });
    } else {
...
module.exports.updateProfile = async (req, res) => {
  try {
    ...
    if (updatedUser) {
      ...
      res.json({
        message: "No user found or unable to update profile",
        status: false,
      });
    }
  } catch (error) {
...
module.exports.appAddtoUser = async (req, res) => {
  try {
    ...
    if (user) {
      res.json({
        message: "App added to user successfully",
        status: true,
        user: user,
      });
    } else {
      res.json({
        message: "App already exists in user's downloaded apps",
        status: true,
        user: null,
      });
    }
...
module.exports.fetchUserInstalledApps = async (req, res) => {
  try {
    ...
    if (!Array.isArray(downloadedAppsIds) || downloadedAppsIds.length === 0) {
      return res.json({
        message: "User has no installed apps",
        status: true,
        apps: [],
      });
    }
    ...
    res.json({
      message: "User installed apps fetched successfully",
      status: true,
      apps: installedApps,
    });
  } catch (error) {
